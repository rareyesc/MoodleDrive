...
@Controller
@RequestMapping("/mainP")
public class ServletProfesor {
    private static final Logger logger = LogManager.getLogger(ServletProfesor.class);
    @Autowired
    private PerfilService perfilService;
    // Anotación @GetMapping indicando que el siguiente método manejará las solicitudes GET a la ruta "/Profesor" dentro del prefijo "/mainP".
    ...
    @GetMapping("/profesor")
    public String mostrarVistaProfesor(@NotNull Model model, @AuthenticationPrincipal UserDetails userDetails) {
        if (userDetails != null) {
